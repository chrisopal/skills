...
1. **Discover the backend and normalize the input**: first check for Codex's built-in `image_gen.imagegen`. Other agents then discover tools, skills, plugins, MCP/connectors, or image models and require prompt-to-image, reference-image editing, and explicit local output. If none qualifies, select `editppt image` with default `gpt-image-2`. Then create an isolated task directory, normalize the input into `pages/page_NNN/source.png`, and record the run backend contract.
