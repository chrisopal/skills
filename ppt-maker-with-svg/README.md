...
To update later (Option A / B): `python3 skills/ppt-master-plus/scripts/update_repo.py`
...
> /plugin install ppt-master-plus@ppt-master-plus
> ```
>
...
```bash
mkdir -p ~/.ppt-master
cp /path/to/installed/ppt-master-plus/.env.example ~/.ppt-master/.env
```

PPT Master reads the current process environment first, then the first `.env` found in this order: current working directory, skill directory (e.g. `~/.agents/skills/ppt-master-plus/.env`), clone repo root, `~/.ppt-master/.env`.

**A) AI generation** — `image_gen.py`. Set `IMAGE_BACKEND` plus the provider's `*_API_KEY` (`OPENAI_API_KEY`, `GEMINI_API_KEY`, etc.), and the pipeline calls it automatically. Run `python3 skills/ppt-master-plus/scripts/image_gen.py --list-backends` for the full backend list. `gpt-image-2` is currently the best default.
...
> Full reference: [`image-generator.md`](./skills/ppt-master-plus/references/image-generator.md) (AI) · [`image-searcher.md`](./skills/ppt-master-plus/references/image-searcher.md) (web).
...
| 📖 | [SKILL.md](./skills/ppt-master-plus/SKILL.md) | Core workflow and rules |
| 📐 | [Canvas Formats](./skills/ppt-master-plus/references/canvas-formats.md) | PPT 16:9, Xiaohongshu, WeChat, and 10+ formats |
| 🛠️ | [Scripts & Tools](./skills/ppt-master-plus/scripts/README.md) | All scripts and commands |
