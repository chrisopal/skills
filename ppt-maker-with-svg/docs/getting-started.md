...
For the first, give the AI your `.pptx` plus your material (or a topic) and ask it to "fill this deck with the new content" — see the [template-fill workflow](../skills/ppt-master-plus/workflows/template-fill-pptx.md). The rest of this section covers create-template.
...
| Location | Path | Notes |
|---|---|---|
| **Registered in the skill library** | `skills/ppt-master-plus/templates/layouts/<id>/` | Global, reusable across every project; run `register_template.py` so it shows up when you ask "what templates are available?" |
...
```
You: Make a deck from sources/report.pdf with template skills/ppt-master-plus/templates/layouts/academic_defense/
```
...
Full guide → [Live Preview Workflow](../skills/ppt-master-plus/workflows/live-preview.md)
...
Full guide → [Animations & Transitions](../skills/ppt-master-plus/references/animations.md)
...
|---|---|
| The AI drifts or forgets a step | Ask it to re-read `skills/ppt-master-plus/SKILL.md`. |
