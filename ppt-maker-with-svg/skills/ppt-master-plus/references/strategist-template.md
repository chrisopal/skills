...
**PPT Master Plus Huixin fit rule**: a selected `huixin_*` Deck establishes the
Huixin identity and narrative language but never forces a page into an
undersized prototype. Map a page to an existing SVG only when its narrative
role, information density, and content capacity fit. If no prototype fits,
author a custom flat SVG for that page while preserving the installed Huixin
font stack, current logo treatment, blue/green color semantics, light
background/chrome rhythm, footer/header geometry, and the selected Deck's topic
logic. A custom Huixin page is an intentional application decision, not a
`no-template-match` failure.
