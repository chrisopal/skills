...
def _validate_svg_template_spec(
    ...
    huixin_legacy_flat = (
        kind == "deck"
        and template_id.startswith("huixin_")
        and frontmatter.get("native_structure_mode") == "legacy-flat"
    )
    if (
        frontmatter.get("native_structure_mode") != "structured"
        and not huixin_legacy_flat
    ):
        errors.append(
            "frontmatter native_structure_mode must be 'structured' "
            "(registered huixin_* Decks may use 'legacy-flat')"
        )
    ...
    if svg_paths and not huixin_legacy_flat:
        try:
