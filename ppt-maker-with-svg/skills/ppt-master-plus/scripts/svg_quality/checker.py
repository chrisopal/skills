...
# native_structure_mode: structured declaration. The registered huixin_* Decks
# retain a narrow legacy-flat compatibility lane while they are migrated page
# by page; every other legacy template-mode package fails closed.
...
def _is_huixin_legacy_flat_template(target_path: Path) -> bool:
    """Return whether a registered Huixin Deck uses the scoped legacy lane."""
    directory = target_path.parent if target_path.is_file() else target_path
    if not directory.name.startswith('huixin_'):
        return False
    spec_path = directory / 'design_spec.md'
    try:
        text = spec_path.read_text(encoding='utf-8')
    ...
    mode = re.search(
        r'^native_structure_mode:\s*legacy-flat\s*$',
        frontmatter,
        re.MULTILINE,
    )
    kind = re.search(r'^kind:\s*deck\s*$', frontmatter, re.MULTILINE)
    deck_id = re.search(
        r'^deck_id:\s*([A-Za-z0-9_-]+)\s*$',
        frontmatter,
        re.MULTILINE,
    )
    return bool(
        ...
        and kind
        and deck_id
        and deck_id.group(1) == directory.name
    )
...
class SVGQualityChecker:
    ...
    def _check_module_bounds_contract(
        ...
        require_bounds = (
            (self.template_mode and not _is_huixin_legacy_flat_template(
                getattr(self, '_active_svg_path', Path('.'))
            ))
    ...
    def check_directory(self, directory: str, expected_format: str = None) -> List[Dict]:
        ...
        for svg_file in svg_files:
            self._active_svg_path = svg_file
            self._active_prototype_path = self._prototype_by_output.get(
                svg_file.resolve()
            )
    ...
    def _check_pptx_structure_contract(
        ...
        if standard_project and declared_mode != 'structured':
            ...
            self._pptx_structure_issues.append((
                ...
                'workspace through skills/ppt-master-plus/workflows/create-template.md, '
    ...
    def _check_template_contract(
        ...
        *,
        ...
        huixin_legacy_flat = _is_huixin_legacy_flat_template(dir_path)
        if declared_structure_mode != 'structured' and not huixin_legacy_flat:
        ...
        if check_structure:
            ...
            if native_contract_path.exists() or source_template_path.exists():
                ...
                self._template_issues.append((
                    ...
                    "skills/ppt-master-plus/workflows/create-template.md",
                ))
            ...
            if legacy_structure_detected:
                self._template_issues.append((
                    ...
                    "workspace through skills/ppt-master-plus/workflows/"
