...
**Default — library scope**: Write `skills/ppt-master-plus/templates/<kind_dir>/<template_id>/` and register it in the matching discovery index.
...
|---|---|---|---|
| `library` (default) | `skills/ppt-master-plus/templates/<kind_dir>/<template_id>/` | `<template_workspace>/templates/` | Run `register_template.py` against the matching global index |
...
```bash
python3 skills/ppt-master-plus/scripts/svg_authoring_view.py "<import_workspace>/svg" -o "<import_workspace>/authoring-svg" --projection-kind layered
```

Only when the import explicitly used `--inheritance-mode both`, create the optional complete-page verification IR:

```bash
python3 skills/ppt-master-plus/scripts/svg_authoring_view.py "<import_workspace>/svg-flat" -o "<import_workspace>/authoring-svg-flat" --projection-kind flat
```
...
```bash
python3 skills/ppt-master-plus/scripts/svg_authoring_view.py "<import_workspace>/authoring-svg" --refresh-summary
```
...
python3 skills/ppt-master-plus/scripts/extract_svg_assets.py "<import_workspace>/authoring-svg" --icons-dir "<import_workspace>/icons" --icon-namespace imported --inplace --id-prefix layered --min-decoration-bytes 3000 --clean-stale
...
python3 skills/ppt-master-plus/scripts/extract_svg_assets.py "<import_workspace>/authoring-svg-flat" --icons-dir "<import_workspace>/icons" --icon-namespace imported --reuse-inventory "<import_workspace>/authoring-svg_vector_asset_inventory.json" --inplace --id-prefix flat --min-decoration-bytes 3000 --clean-stale
```
...
```bash
python3 skills/ppt-master-plus/scripts/extract_svg_pictures.py \
...
```bash
python3 skills/ppt-master-plus/scripts/svg_authoring_view.py "<normalized_svg_source>" -o "<svg_analysis_workspace>/authoring-svg" --projection-kind generic
python3 skills/ppt-master-plus/scripts/extract_svg_assets.py "<svg_analysis_workspace>/authoring-svg" --icons-dir "<svg_analysis_workspace>/icons" --icon-namespace imported --inplace --id-prefix source --min-decoration-bytes 3000 --clean-stale
```
...
```bash
python3 skills/ppt-master-plus/scripts/source_to_md.py "<file_or_URL_or_dir>" -o "<text_analysis_workspace>"
```
...
template_workspace="skills/ppt-master-plus/templates/<kind_dir>/<template_id>"
...
|---|---|---|
| `library` | `skills/ppt-master-plus/templates/<kind_dir>/<template_id>/` | Run the common workspace preflight; the directory name matches the final template ID used in the relevant index |
...
```bash
python3 skills/ppt-master-plus/scripts/mirror_template_materialize.py \
...
```bash
python3 skills/ppt-master-plus/scripts/svg_quality_checker.py "<template_workspace>/templates" --template-mode
```

In `library` scope, additionally run the registrar dry-run so `brand_id` is checked against the library directory/index key:

```bash
python3 skills/ppt-master-plus/scripts/register_template.py <brand_id> --kind brand --dry-run
```
...
```bash
python3 skills/ppt-master-plus/scripts/register_template.py <style_id> --kind style --dry-run
```
...
```bash
python3 skills/ppt-master-plus/scripts/compact_svg_coordinates.py "<template_workspace>/templates" --inplace --keep-native-frames
python3 skills/ppt-master-plus/scripts/svg_quality_checker.py "<template_workspace>/templates" --template-mode --format <canvas_format>
```
...
```bash
python3 skills/ppt-master-plus/scripts/template_preview_pptx.py "<template_workspace>"
```
...
```bash
python3 skills/ppt-master-plus/scripts/template_preview_pptx.py "<template_workspace>" --force
```
...
```bash
# For brand
python3 skills/ppt-master-plus/scripts/register_template.py <template_id> --kind brand

# For style
python3 skills/ppt-master-plus/scripts/register_template.py <template_id> --kind style
...
>
> ```bash
> python3 skills/ppt-master-plus/scripts/register_template.py --kind style --rebuild-all
