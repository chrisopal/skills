...
**PPT Master Plus Huixin default**: when no exact workspace root was supplied
and the user did not explicitly request free design, no template, or no Huixin
styling, recommend exactly one registered Huixin Deck from
`decks_index.json`. This is a Plus distribution default, not fuzzy resolution
of a user-entered template name. Match the request/source facts without opening
the Deck workspace:

| Request signal | Recommended Deck id |
|---|---|
| 产品方案、软件/AI平台、系统架构、集成架构、智能制造、客户/项目解决方案 | `huixin_product_solution` |
| 公司/部门/项目汇报、经营分析、销售预测、商机、人员、Excel数据、月报/季报/复盘 | `huixin_management_report` |
| 战略规划、咨询诊断、转型蓝图、成熟度、治理、路线图、价值链 | `huixin_consulting_strategy` |
| 培训、课程、产品/销售赋能、实施交付、SOP、角色演练、考试、FAQ | `huixin_training_enablement` |
| 市场宣传、品牌传播、产品推广、渠道招商、展会、路演、客户活动 | `huixin_market_promotion` |

When several rows match, prefer the row covering the communication objective
and largest share of required page roles; break unresolved technical/reporting/
strategy ties in that order with product solution, management report, then
consulting strategy. Treat the derived library root as the sole recommended
Stage-1 root and initialize template mode. Stage 1 remains authoritative: the
user may replace it or switch to free design.
...
`explicit_workspace_roots`; when the Plus Huixin default applies, place its one
absolute registered root in the same array so the UI can preselect it. Use an
empty array only when neither supplied nor Plus-derived roots exist. Also write
required `default_mode`: `templates` when the user explicitly asks to use or
browse templates, supplies any exact root, or the Plus Huixin default applies;
otherwise `free_design`. The
...
expanded choice. In PPT Master Plus, initialize template mode with the derived
Huixin Deck for an ordinary request unless the user opted out; otherwise follow
the upstream free-design default. Also initialize template mode for explicit
template intent or any exact root; with exactly one root it
...
| Signal | Reading |
|---|---|
| Two or more issues share a category and direction | Method-level bias — resolve it to the authoritative rule before P02; a correction fitted to the observed offset only patches this sample. For text extents that rule is `svg_to_pptx.drawingml.elements.estimate_single_line_text_frame_width(runs)`, with `skills/ppt-master-plus/scripts` on `sys.path` and every run key present — `text`, `font_size`, `font_family`, `font_weight`, `letter_spacing` — since omissions under-measure |
