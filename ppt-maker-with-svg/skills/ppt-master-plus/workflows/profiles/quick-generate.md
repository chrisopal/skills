...
| Templates | Directly validate and install at most one exact workspace root per kind supplied for this run; when none are supplied, PPT Master Plus derives one registered Huixin Deck unless the user opted out, then continues without Confirm UI |
...
- **Free design**: no exact root was supplied and the user opted out of the Plus
  Huixin default (or Image to PPTX owns the page surface). Continue immediately
  with the requested canvas or `ppt169`. A bare non-Huixin template name, brand
  mention, style phrase, or vague request to choose a template is ordinary brief
  input, not a workspace reference.
