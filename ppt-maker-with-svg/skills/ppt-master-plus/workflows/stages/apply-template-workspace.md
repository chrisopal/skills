...
**PPT Master Plus Huixin compatibility exception**: registered legacy-flat Decks
whose `deck_id` starts with `huixin_` remain valid Plus-owned visual/template
inputs while they are migrated incrementally. Validate their roster, assets,
canvas, placeholders, and SVG quality, install them through the legacy-flat
copy path, and expose their prototypes to Strategist as visual/capacity
references. They do not claim reusable native Master/Layout topology: pages
that cannot use a prototype are authored flat under the Huixin identity, and
the resulting lock must not invent structured metadata for those pages. This
exception is narrow to Plus-owned Huixin Decks and must not admit arbitrary
legacy packages.
...
```bash
python3 skills/ppt-master-plus/scripts/svg_quality_checker.py "<workspace_root>/templates" --template-mode
```
...
```markdown
> **Installed from**: `skills/ppt-master-plus/templates/brands/mckinsey/` (library)
```
