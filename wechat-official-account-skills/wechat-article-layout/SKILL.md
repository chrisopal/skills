...
When preparing the homepage cover, keep it simpler than the inline explanatory diagrams:

1. one core scene or 3-5 modules only
2. optional 1-2 short result badges such as `10周上线` or `90%自动通过`
3. no dense bullet lists, capability tables, bottom explanation bars, or multi-zone whitepaper layout
4. let the article title do the attraction work; let body images carry the detailed explanation
...
- when the repo provides a persistence helper, run it immediately after the `imagegen` call; in `/Users/guojiexie/content-mgmt`, default to `python3 scripts/persist_imagegen_output.py --target assets/wechat/<slug>-cover-imagegen.png --record assets/wechat/imagegen-manifest.jsonl`
...
2. Save/copy the selected image into the project workspace, usually `assets/wechat/`. When the repo provides a persistence helper, run it immediately; in `/Users/guojiexie/content-mgmt`, default to `python3 scripts/persist_imagegen_output.py --target assets/wechat/<slug>-<role>-imagegen.png --record assets/wechat/imagegen-manifest.jsonl`.
...
3. If the repo has a persistence helper that can automate the copy/decode step, use it before doing anything manual. In `/Users/guojiexie/content-mgmt`, use `scripts/persist_imagegen_output.py` and let it prefer host cache files first, then session-log base64 decode.
4. If the host call only exposed the image inside the current session record, decode/export that selected result into the workspace and then continue.
5. Only after those steps fail should you consider a documented non-imagegen fallback.
...
- verify each persisted asset has a matching manifest record when the repo defines one, for example `assets/wechat/imagegen-manifest.jsonl`
